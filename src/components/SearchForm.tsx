    ...
    return (
      <form onSubmit={handleSubmit} className="card flex flex-col gap-3 p-4 sm:flex-row sm:items-end">
        ...
        <div className="flex-1">
          ...
          <input type="date" min={today} value={date} onChange={(e) => setDate(e.target.value)} className="input-field" required />
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="card grid grid-cols-1 gap-4 p-6 md:grid-cols-[1fr_auto_1fr_1fr_auto]">
      ...
      <div className="flex items-end justify-center pb-1">
        <button type="button" onClick={swap} title={t('swapBtn')} className="rounded-full border border-slate-200 bg-white p-2.5 text-slate-400 transition-all hover:rotate-180 hover:bg-slate-50 hover:text-[#0066ff]">
          <ArrowLeftRight className="h-4 w-4" />
        </button>
      </div>
      <div>
      ...
      <div>
        ...
        <input type="date" min={today} value={date} onChange={(e) => setDate(e.target.value)} className="input-field text-base font-medium" required />
      </div>
